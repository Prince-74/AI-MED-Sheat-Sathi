...
export async function analyzeWithOpenAI(ocrText) {
  ...
  try {
    ...
    if (allowFallback) {
      console.warn("OpenAI analysis failed, using OCR-only fallback:", err?.message || err);
    }
    if (allowFallback) {
      return {
...
export async function analyzeWithGemini(ocrText) {
  ...
  try {
    ...
    const allowFallback = String(process.env.FALLBACK_OCR_ONLY || "").toLowerCase() === "true";
    if (allowFallback) {
      console.warn("Gemini analysis failed, using OCR-only fallback:", error?.message || error);
    }
    if (allowFallback) {
      return {
        summary: "AI analysis temporarily unavailable. Showing OCR text only.",
